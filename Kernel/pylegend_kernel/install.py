...
def create_kernelspec(dest_dir):
    with open(os.path.join(dest_dir, 'kernel.json'), 'w', encoding='utf-8') as f:
        kernel_json = {
            'argv': [sys.executable, '-m', 'pylegend_kernel', '-f', '{connection_file}'],
...
def main(argv=None):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        KernelSpecManager().install_kernel_spec(temp_dir, 'pylegend_kernel', user=args.user, prefix=args.prefix)
